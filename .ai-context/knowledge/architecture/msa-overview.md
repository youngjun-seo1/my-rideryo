# 🏗️ Rideryo MSA 아키텍처

## 서비스 구성

```
                    ┌─────────────────┐
                    │  API Gateway    │
                    │  (gateway-api)  │
                    └────────┬────────┘
                             │
        ┌────────────────────┼────────────────────┐
        │                    │                    │
        ▼                    ▼                    ▼
┌───────────────┐   ┌───────────────┐   ┌───────────────┐
│ rider-service │   │delivery-service│   │ order-service │
│   (라이더)     │   │   (배달)       │   │   (주문)      │
└───────┬───────┘   └───────┬───────┘   └───────┬───────┘
        │                   │                   │
        └───────────────────┼───────────────────┘
                            │
                            ▼
                 ┌─────────────────────┐
                 │notification-service │
                 │      (알림)         │
                 └─────────────────────┘
```

## 서비스별 책임

### 1. gateway-api
- **역할**: API 라우팅, 인증/인가
- **포트**: 8080
- **기술**: Spring Cloud Gateway

### 2. rider-service
- **역할**: 라이더 등록, 상태 관리, 시프트 관리
- **포트**: 8081
- **DB**: PostgreSQL (rider_db)
- **주요 API**:
  - `POST /riders` - 라이더 등록
  - `PUT /riders/{id}/status` - 상태 변경
  - `GET /riders/available` - 가용 라이더 조회

### 3. delivery-service
- **역할**: 배달 생성, 배차, 배달 상태 관리
- **포트**: 8082
- **DB**: PostgreSQL (delivery_db)
- **주요 API**:
  - `POST /deliveries` - 배달 생성
  - `POST /deliveries/{id}/assign` - 배차
  - `PUT /deliveries/{id}/status` - 상태 변경

### 4. order-service
- **역할**: 주문 수신, 가맹점/고객 정보 관리
- **포트**: 8083
- **DB**: PostgreSQL (order_db)
- **주요 API**:
  - `POST /orders` - 주문 생성
  - `GET /orders/{id}` - 주문 조회

### 5. notification-service
- **역할**: 푸시 알림, SMS, 카카오 알림톡
- **포트**: 8084
- **기술**: Firebase FCM, AWS SNS
- **주요 API**:
  - `POST /notifications/push` - 푸시 발송
  - `POST /notifications/sms` - SMS 발송

## 서비스 간 통신

### 동기 통신 (REST)
- Gateway → 각 서비스
- delivery-service → rider-service (라이더 조회)

### 비동기 통신 (Event)
- **Message Broker**: Apache Kafka
- **토픽 설계**:
  - `rider.status.changed`
  - `delivery.requested`
  - `delivery.assigned`
  - `delivery.completed`
  - `order.received`

## 데이터 흐름

### 배달 요청 흐름
```
1. order-service: 주문 수신 → OrderReceived 이벤트 발행
2. delivery-service: 이벤트 수신 → 배달 생성
3. delivery-service: 최적 라이더 조회 (rider-service)
4. delivery-service: 배차 → DeliveryAssigned 이벤트 발행
5. notification-service: 라이더에게 푸시 알림
6. rider-service: 라이더 상태를 DELIVERING으로 변경
```

## 인프라 구성

- **컨테이너**: Docker + Kubernetes
- **서비스 디스커버리**: Kubernetes Service
- **설정 관리**: ConfigMap, Secrets
- **로깅**: ELK Stack
- **모니터링**: Prometheus + Grafana
- **추적**: Jaeger (분산 추적)

